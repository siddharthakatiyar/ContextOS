...
async function runContextOSFlowB(topic) {
  const calls = [];
  ...
  while ((mMatch = re.exec(output)) !== null) {
    ...
    if (!filesRead.includes(rel)) {
      allStubs.push(rel);
      filesRead.push(rel);
    }
  }

  // 1. ctx_expand on cited files using signalTerms
  for (const rel of allStubs) {
    // We simulate ctx_expand by using sandboxExtract with signal terms
    const extract = sandboxExtract(rel, signalTerms, 20);
    if (!extract.exists) continue;
    
    // Fall back to full file read if extract is tiny (e.g. signal terms weren't exactly in file)
    // Actually, in ctx_expand we'd expand around the chunk window. For now, sandboxExtract is fine.
    accumulated += `\n\n----- ctx_expand ${rel} -----\n` + extract.text;
    calls.push({ tool: "ctx_expand", tokens: extract.tokens, path: rel });
  }
