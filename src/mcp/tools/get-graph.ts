    ...
    async ({ symbolName }) => {
      try {
        ...
        if (chunks.length === 0 && relationships.length === 0) {
          return { content: [{ type: "text", text: `No code or relationships found for symbol: ${symbolName}` }] };
        }
        ...
        if (chunks.length > 0) {
          ...
          if (chunks.length > 1) {
            ...
            for (let i = 1; i < chunks.length; i++) {
               const c = chunks[i];
               const loc = c.startLine != null && c.endLine != null ? `${c.sourceFile}:${c.startLine}-${c.endLine}` : c.sourceFile;
        ...
        if (tokens > 1000) {
           ...
           const trimmed = lines.slice(0, 150).join('\n') + `\n\n... [truncated to fit 1000-token cap] ...`;
           output = trimmed;
        }
