...
export function registerKnowledgeTools(server: McpServer, dbs: DB[]) {
  ...
  server.tool(
    ...
    {
      type: z.enum(["session_note", "cross_session_fact", "forget_fact"]).describe("Type of memory operation"),
    ...
    async ({ type, content, category }) => {
      try {
        if (type === "forget_fact") {
          ...
          return { content: [{ type: "text", text: `Successfully saved context note to session memory.` }] };
        }
      } catch (error: any) {
        return {
...
export function registerLegacyKnowledgeTools(server: McpServer, dbs: DB[]) {
  ...
  server.tool(
    ...
    "Store a cross-session memory fact. Use this to remember user preferences, architectural decisions, or recurring bugs across different chat sessions.",
    {
      ...
      category: z.string().optional().default("general").describe("An optional category for the fact"),
    },
    async ({ fact, category }) => {
      try {
