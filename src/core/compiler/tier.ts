...
export function assessTier(result: RetrievalResult): 'exact' | 'exact-implementation' | 'file' | 'explore' {
  const topChunks = result.chunks.slice(0, 5);
  
  const exactMatch = topChunks.some(c => {
    return c.symbolName && 
           result.intent?.identifiers?.includes(c.symbolName) && 
           c.score >= 10;
  });

  if (exactMatch) {
    const rawPrompt = result.intent?.rawPrompt || '';
    const isImplementation = /(how|what) does .* (work|function|do)/i.test(rawPrompt) || 
